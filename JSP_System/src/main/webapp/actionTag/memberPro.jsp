...
<body>
	...
	<br>
	<!-- 
		useBean 액션 태그로 new 사용하지 않고 객체 만들기
		id 속성에는 만드려는 객체의 이름을 쓴다.
		class 속성에는 객체를 만드려하는 클래스의 이름을 반드시 풀 패키지 이름과 같이 써야한다.
		액션 태그는 xml문법을 따르기 때문에 <tag> </tag> 사이에 아무런 내용이 코딩되지 않으면
		</tag> 생략할 수 있는데 그냥 지우면 에러가 발생하니 <tag /> 만들면 된다.
	 -->
	 
	 <jsp:useBean id="vo" class="members.MemberVO">
	 <!-- 
	 	setProperty 액션태그는 지정한 필드의 setter 메서드를 호출한다.
	 	Property 속성에는 setter 메서드를 실행할 필드이름을 쓴다.
	  ...
	  단, form에서 넘어온 name 속성하고 필드의 이름이 같은 것만 setter메서드가 실행된다.
	 -->

...
<body>
	...
	int startNo = 0; // 현재 브라우저에 표시되는 글의 시작 인덱스 번호 -> mysql 0부터 시작
	...
	// 현재 사용하는 테이블 4명기본적용 물어볼 필요가 없다.
	rs.next();
	totalCount = rs.getInt(1);
	// out.println("테이블 저장된 전체 글의 개수:" +totalCount+ <br/>");

	// totalPage변수에 전체 페이지 개수를 계산해서 저장한다.
	...
	try {
		...
		// 번호가 전체 페이지 개수보다 큰 값이 넘어왔다면 화면에 표시할 페이지 번호를 
		// 전체 페이지 개수로 수정한다. 
	...
	// mysql select 했을 때 맨 처음 나오는 글의 번호가 0번 오라클 1부터 시작된다.
 ...
 %>
				<button type="button" onclick="location.href='?currentPage=<%=i%>'">
					<%=i%></button> <%
 }
 }
 %>

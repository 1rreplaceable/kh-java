      ...
      try{
         while(true){
            ...
            switch(command){
               ...
               // 대화방 참가 시도 메시지  PACKET : REQ_ENTERROOM|ID|ROOMID
               case REQ_ENTERROOM:{
                  st_buffer.setLength(0);
                  String id = st.nextToken(); // 클라이언트의 ID를 얻는다.
                  ...
                  if(userRoomVector.contains(roomname)) {
                	  ...
                	  //MDY_ROOMUSERS PACKET : MDY_ROOMUSERS|ROOOMID|각방에있는IDid1'id2' ....
                	  st_buffer.setLength(0);
               ...
               // 방에 아무도 없을경우 전송 시도 메시지 PACKET : YES_QUITROOM|ZERO
               case REQ_QUITROOM:{
            	  String id = st.nextToken();

...
//	저장된 데이터는 저장할 데이터 타입으로 반드시 형변환시켜서 사용해야 한다.
	...
	//  메서드는 request 영역의 데이터를 가지지 않고 인수로 지정된 페이지로 넘어간다.
	//  메서드는 주소창에 표시되는 페이지 이름이 인수로 지정된 페이지 이름으로 변경된다.
	//  현재 페이지와 sendRedirect() 메서드의 인수로 지정한 페이지가 완전히 분리된다.
	//  새로고침을 하면 주소창에 표시된 페이지만 다시 실행된다.
	response.sendRedirect("requestTest.jsp");
	
	// forward() : 인수로 지정된 페이지로 넘겨준다.
	// 	메서드는 request 영역의 데이터를 가지고 인수로 지정된 페이지로 넘어간다.
	//  메서드는 주소창에 표시되는 페이지 이름이 forwatd() 메서드가 실행된 페이지 이름을
	//  그대로 유지한다.
	//	현재 페이지와 forward() 메서드의 인수로 지정한 페이지가 연결된 상태다.
	//	새로고침을 하면 주소창에 표시된 forward메서드가 입력 된 페이지부터 현재 화면에 보이는 페이지
	//	까지 실행이된다.

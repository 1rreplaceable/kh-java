...
<body>
	...
	int allArticlesCount = BoardDAO.getInstance().getBoardlistSize(); //전체 게시글 수
	...
	<div align="center">
		<h2>
			게시글 목록(<%=allArticlesCount%>개)
		</h2>
		<form method="get" action="_12updateBoardSize.jsp">

			<table border="1">
				<tr>
					<td colspan="5">한 페이지에 보여줄 게시글 수 <select
						name="onePageArticlesCount">
				...
				<tr>
					...
					<td><a href="_05boardInfo.jsp?num=<%=board.getNum()%>"> <%=board.getSubject()%>
					</a></td>
				</tr>
			...
			%>
			[<a
				href="_04_01boardList.jsp?currentPageNumber=<%=startPageNum - clickablePageCount%>">이전</a>]
			<%
			}
			%>
			<%
			...
			%>
			[<a href="_04_01boardList.jsp?currentPageNumber=<%=i%>"><%=i%></a>]
			<%
			}
			%>
			<%
			...
			%>
			[<a href="_04_01boardList.jsp?currentPageNumber=<%=endPageNum + 1%>">이후</a>]
			<%
			}
			%>

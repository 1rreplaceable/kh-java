...
<body>
	<!-- 
		onsubmit 이벤트는 form의 submit 버튼이 클릭되면 실행되는 이벤트
		form check는 submit 버튼이 클릭되면 실행되는 onsubmit 이벤트에서 javascript
		함수를 실행해서 form에 입력된 데이터가 정상적인지 유효성 검사해서 결과가
		정상 true, 비정상 false 리턴
		
		true값이 오면 action 속성으로 지정한 페이지를 넘겨주고 false 현재 페이지
		그대로 머물게 한다.
	...
	<form action="?" method="post" onsubmit="return passwordCheck(this)">
		...
		<!-- getRemoteAddr() 메서드로 접속자 ip주소를 얻어 올 수 있다. -->
